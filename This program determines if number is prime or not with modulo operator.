...
int main(void)
{
	...
	for (c = 2; c <= n - 1; c++)    //loops as long as c <= n-1 since n has to be greater than c to be divided by it
	{
		...
		{                                //if condition is only entered when the remainder is 0; if remainder not 0, 
						//jumps back to for loop
	...
	if (c == n)                              //once the for loop is exahausted and if condition is never entered.
						//we conclude prime
		printf("%d is prime.\n", n);
